...
tempToRGB(temp) {
  ...
  // Fahrenheit zu Celsius konvertieren
  if (fahrenheit) {
    temp = (temp - 32) * 5 / 9;
  }
  
  // Spezielle Behandlung für Luftfeuchtigkeit (bleibt unverändert)
  if (humidity) {
  ...
  // NEUE FARBSKALA: -50 bis +50 = Grün-Bereich
  // < -50: Grün → Gelb (kälter)
  // -50 bis +50: Grün (Neutralbereich)
  // > +50: Grün → Rot (wärmer)
  
  const GREEN_START = -50;  // Untere Grenze des grünen Bereichs
  const GREEN_END = 50;     // Obere Grenze des grünen Bereichs
  
  if (temp < GREEN_START) {
    // BEREICH 1: temp < -50 → Grün zu Gelb
    // Je kälter, desto gelber
    var coldRange = Math.abs(temp - GREEN_START);
    var coldIntensity = Math.min(coldRange / 50, 1); // Normalisiert auf 0-1
    
    var r = parseInt(255 * coldIntensity);  // Rot-Anteil steigt
    var g = 255;                             // Grün bleibt voll
    var b = 0;                               // Kein Blau
  ...
  else if (temp >= GREEN_START && temp <= GREEN_END) {
    // BEREICH 2: -50 bis +50 → Grün
    return "00ff00";
  } 
  else {
    // BEREICH 3: temp > +50 → Grün zu Rot
    // Je wärmer, desto röter
    var hotRange = temp - GREEN_END;
    var hotIntensity = Math.min(hotRange / 50, 1); // Normalisiert auf 0-1
    
    var r = parseInt(255 * hotIntensity);    // Rot-Anteil steigt
    var g = parseInt(255 * (1 - hotIntensity)); // Grün-Anteil sinkt
    var b = 0;                                // Kein Blau
